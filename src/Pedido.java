...
public class Pedido implements Comparable<Pedido>{
	...
	public double valorFinal() {
		...
		// Soma o valor de todos os itens (preço congelado * quantidade)
		for (int i = 0; i < quantItens; i++) {
			valorPedido += itens[i].getPrecoVenda() * itens[i].getQuantidade();
		}
		
		// Aplica desconto se pagamento à vista (formaDePagamento == 1)
		if (formaDePagamento == 1) {
			valorPedido = valorPedido * (1 - DESCONTO_PG_A_VISTA);
		}
    ...
    public double valorCatalogoAtual() {
        ...
        for (int i = 0; i < quantItens; i++) {
            // Usa o valor de venda ATUAL do produto (não o congelado)
    ...
    public double indiceEconomia() {
        // Índice de economia = valorCatalogoAtual - valorFinal
